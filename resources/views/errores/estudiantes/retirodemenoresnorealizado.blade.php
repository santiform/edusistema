@include('estudiantes.includes.headerestudiantes')
  
            <center>
          <h1>Todavía no podés usar nuestro Sistema, porque están incompleto los Datos de retiro de menores.</h1>
          <p>(Recordá que debe ser llenado por un mayor a cargo).</p>
          
          <div style="height: 1rem" ></div>

          <a class="botonformulario" href="https://localhost/edusistema/public/estudiantes/inscripciones/reinscripciones/retiro-de-menores">
                <span style="vertical-align: middle; margin-top: -0.2rem;" class="material-icons">
                    edit_note
                </span>
                Completar
            </a>
        </center>
        
<div style="height: 1rem" ></div>


<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/js/bootstrap.min.js"></script>
<script src="https://localhost/edusistema/public/node_modules/sweetalert2/dist/sweetalert2.all.min.js"></script>

</body>
</html>
