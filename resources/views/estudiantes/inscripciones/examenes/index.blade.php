@include('estudiantes.includes.headerestudiantes')

          <h1 class="black">- Inscripción a EXAMENES</h1>

          <div style="height: 1rem" ></div>

          <p>Recordá, que el formulario para agregar inscripciones a exámenes estará <b>disponible</b> durante <b>febrero/marzo</b>, <b>julio/agosto</b> y <b>diciembre</b> de cada año.</p>
          
          <div style="height: 0.001rem" ></div>


          		<a href="https://localhost/edusistema/public/estudiantes/inscripciones/examenes/mis-inscripciones">
			  <button class="boton2">
			    <span class="material-icons" style="font-size: 1.4em; vertical-align: middle;">arrow_forward</span>
			    <span style="font-size: 19px; margin-inline: 0.4rem; vertical-align: middle;">Mis inscripciones </span>
			  </button>
			 </a>


	         <a href="https://localhost/edusistema/public/estudiantes/inscripciones/examenes/formulario">
			  <button class="boton2">
			    <span class="material-icons" style="font-size: 1.4em; vertical-align: middle;">arrow_forward</span>
			    <span style="font-size: 19px; margin-inline: 0.4rem; vertical-align: middle;">Completar formulario </span>
			  </button>
			 </a>


			 <div style="height: 2rem;" ></div>

          <br>

<p style="color: transparent;" > EduSistema ESTUDIANTES - EduSistema ESTUDIANTES - 
	EduSistema ESTUDIANTES	-! </p>
	
@include('estudiantes.includes.footerestudiantes')