...
class ParteEstudianteController extends Controller
{

    public function esMenorDeEdad()
{       
    $dni = auth()->user()->dni;
    $fechaNacimiento = Estudiante::where('dni', $dni)->value('nacimiento');
    $fechaNacimientoCarbon = Carbon::parse($fechaNacimiento); // Convierte la cadena a un objeto Carbon
    $fechaActual = Carbon::now(); // Obtiene la fecha actual como un objeto Carbon
    $edad = $fechaNacimientoCarbon->diffInYears($fechaActual); 
    return $edad < 18; // Devuelve true si el usuario es menor de 18 años, false en caso contrario
}

    public function formRetiro()
{       
    $dni = auth()->user()->dni;
    $ruta = storage_path('app/formularios/retiro-de-menores/' . $dni . '.csv');
    if (File::exists($ruta)) {
...
{       
    ...
    public function inicio()
    {
        ...
        if ($menor === true && $retiromenores === false) {
           return view('errores.estudiantes.retirodemenoresnorealizado');
        }
    ...
    public function miPerfil(){
        ...
        $datos = Estudiante::where('dni', $dni)->first();

        return view('estudiantes.miperfil', compact('datos', 'edad'));
    ...
    public function miPerfilEditar(Request $request){
        ...
        return redirect()->route('miperfil')->with('editarPerfil', 'ok');;
...
public function inscripcionMaterias()
{
    ...
    return view('estudiantes.inscripciones.materias.formulario', compact('catedrasDisponibles'));
}
...
public function inscripcionExamenes()
{
    ...
    return view('estudiantes.inscripciones.examenes.formulario', compact('examenesDisponibles'));
}
...
public function misInscripcionesMaterias()
{
    ...
    return view('estudiantes.inscripciones.materias.misinscripciones', compact('inscripciones'));
...
public function misInscripcionesExamenes()
{
    ...
    return view('estudiantes.inscripciones.examenes.misinscripciones', compact('inscripciones'));
...
public function misCalificaciones() {
    ...
    // Obtener el DNI del estudiante autenticado
    $dniEstudiante = auth()->user()->dni;
    ...
    foreach ($calificaciones as $calificacion) {
        ...
        return view('estudiantes.historialacademico.miscalificaciones', compact('calificaciones', 'nombreCarrera'));
...
public function avanceEnLaCarrera() {
    ...
    $idCarrera = DB::table('estudiantes')
        ->where('dni', $dni)
        ->value('carrera');
    ...
    return view('estudiantes.historialacademico.avanceenlacarrera', compact('nombreCarrera', 
...
public function planDeEstudio() {
    ...
    return view('estudiantes.informacionacademica.plandeestudio', compact('materias', 'nombreCarrera')); 
...
public function certificadoEstudianteRegular() {
    ...
    $pdf = PDF::loadView('estudiantes.informacionacademica.certificadoestudianteregular', $data);
